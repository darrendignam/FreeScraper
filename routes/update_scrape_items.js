...
router.get('/', function(req, res) {
      ...
      stream.on('data', function (doc) {
        ...
        //if(doc.url == 'https://groups.freecycle.org/group/HackneyUK/posts/60020864/3-4%20white%20gloss%20internal%20doors'){
          //console.log(doc.url); 



          //check for this page being scraped already... if not - scrape it - only scrape new pages for now ;)
          FreeItem.findOne({url: doc.url}, function(err,obj) { 
            if(!err && !obj.description){
                scrapeItemPage(doc.url);
                //console.log("Found: %j", obj);
            }//else there is a description, so we already scanned it, so dont scan again....
          });
